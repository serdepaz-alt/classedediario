...
import { Calendar, Loader2, CalendarRange, ChevronDown, ChevronUp, FileDown, User } from "lucide-react";
...
export const CronogramaView = () => {
  ...
  const getVisibleDisciplinas = (items: DisciplinaRow[], turmaId: string) => {
    if (expandedTurmas[turmaId]) {
      const currentIdx = items.findIndex((d) => {
        try {
          return isWithinInterval(new Date(), { start: parseISO(d.data_inicio), end: parseISO(d.data_termino) });
        } catch { return false; }
      });
    ...
    if (allFuture) return { visible: items.slice(0, 3), currentIdx: -1, startIdx: 0 };

    if (currentIdx === -1) currentIdx = items.findIndex((d) => d.data_inicio > today);
    if (currentIdx === -1) currentIdx = Math.max(0, items.length - 2);
  ...
  const handleExportPDF = () => {
    ...
    Object.entries(turmaGroups).forEach(([, items]) => {
      ...
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <CardDescription>
              Visualização dos cronogramas de disciplinas por turma
            </CardDescription>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-md">
              Configure os cronogramas no módulo de Configurações.
            </p>
          </div>
        ) : (
